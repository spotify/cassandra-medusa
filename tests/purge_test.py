...
class PurgeTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        config['storage'] = {'host_file_separator': ',', 'storage_provider': 'local', 'base_path': '/tmp',
    ...
    def make_blob(self, blob_name, blob_date):
        extra = {}
        extra['creation_time'] = blob_date
        extra['access_time'] = blob_date
        extra['modify_time'] = blob_date
        checksum = hashlib.md5()
        checksum.update(os.urandom(4))
        return Object(name=blob_name, size=randrange(100), extra=extra,
                      driver=self, container=None, hash=checksum.hexdigest(),
                      meta_data=None)
